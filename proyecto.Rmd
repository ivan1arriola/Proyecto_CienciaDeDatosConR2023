...
transformarCoord <- function(lat, lon, mvd_map){
  ...
  puntos_sf <- st_as_sf(puntos_lat_lng, coords = c("lng", "lat"), crs = 4326)
  puntos_transformados <- st_transform(puntos_sf, crs = st_crs(mvd_map))
...
```{r}
velocidadBarrioMedia %>% ggplot(
      aes(
          y = promedio_velocidad,
...
encontrar_barrio <- function(lat, lon, mvd_map) {
  ...
  ls_contains <- st_contains(mvd_map$geometry, puntos_transformados$geometry)
