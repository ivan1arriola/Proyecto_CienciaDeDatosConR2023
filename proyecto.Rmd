...
```{r Cantidad de datos por ubicacion ponderado por area 2, fig.cap = 'Cantidad de datos por ubicacion ponderado por area'}
...
```{r Grafico de , fig.cap = "Es netamente nula la proporcion de excesos de velocidad, podemos decir el los conductores en general respetan los limites de velocidad"}
excesos %>% 
...
```
La velocidad parece no variar a lo largo de los días de la semana. Alcanzan su valor máximo alrededor de los 35km/h.
...
Durante el fin de semana las velocidades máximas durante el horario laboral de lunes a viernes es totalmente diferente, los conductores durante el fin de semana aprovechan la baja de volumen para circular mas rápido. También se observa un comportamiento distinto los fines de semana, ya que hay muchos más casos atípicos de valores inferiores; esto puede ser causado por la sensación de relax que suele haber en los fines de semana, por no ser días laborables.
...
Además, podemos ver que los momentos de mayor volumen de tráfico suelen darse a la tarde, entre las 16:00 y las 18:00. También, los momentos donde el volumen es menor, y los picos de velocidad mayores suelen darse en la madrugada, entre las 2:00 y las 4:00
...
FROM fct_registros TABLESAMPLE SYSTEM (0.1)
LEFT JOIN d_date ON fct_registros.id_fecha = d_date.id_fecha
GROUP BY
    fct_registros.id_detector,
    fct_registros.id_carril,
...
La aplicación consta de 4 partes. Un mapa con la ubicación de los sensores, un analisis univariado y otro multivariado.
La aplicación resume todo lo hecho anteriormente.
