...
Que velocidad promedio se espera según 
- es fin de semana
- es día o noche
- volumen promedio
...
datos_arbol <- DBI::dbGetQuery(
  ...
  SELECT
    ...
    CASE
            WHEN fct_registros.id_hora >= 800 AND fct_registros.id_hora < 2000 THEN TRUE
            ELSE FALSE
    END AS esDeDia
FROM fct_registros
LEFT JOIN d_date ON fct_registros.id_fecha = d_date.id_fecha
...
```{r, eval=FALSE}
intrain <- sample(x = 1:nrow(datos_arbol), size = nrow(datos_arbol)*0.9)
...
testing$pred_avg_velocidad <- predict(arbol_vol, newdata = testing)

# Calculate the mean squared error (MSE) to measure the accuracy of the model
(
  mse <- mean((testing$avg_velocidad - testing$pred_avg_velocidad)^2)
)
