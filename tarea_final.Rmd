...
```{r Tabla, echo = FALSE}
registros_max_barrioxdiaxhora <- load_data("registros_max_barrioxdiaxhora.csv",
    con,
    "
      SELECT
        d_date.day_of_week,
...
```{r Grafico de distribucion max_velocidad, echo = FALSE}
...
```{r Datos para densidad de velocidad, echo = FALSE}
tabla  %>% 
  mutate( 
    dia_de_la_semana = 
      factor(
...
```{r Arbol Plot, eval=FALSE}
rpart.plot(arbol_vol, digits = (-5))

```

```{r Arbol testing, eval=FALSE}
...
```{r Datos Random Forest, eval=FALSE}
